...
macro_rules! cilk_parse_ty {
    ...
    ($tys:expr, ptr $($elem:tt)*) => {{
        let e = $crate::cilk_parse_ty!($tys, $($elem)*);
        $tys.new_pointer_ty(e)
    }};
    ($tys:expr, [$n:expr; $ty:ident]) => {{
        let e = { $crate::cilk_parse_ty!($tys, $ty)};
...
macro_rules! cilk_expr {
    ...
    ($builder:expr; $bb_map:expr; $x:ident = call $name:ident [$( ( $($arg:tt)* ) ),*] ; $($remain:tt)*) => {
        let args = vec![ $( $crate::cilk_value!($builder; $( $arg )*) ),* ];
    ...
    ($builder:expr; $bb_map:expr; br ($($cond:tt)*) $l1:ident, $l2:ident; $($remain:tt)*) => {
        ...
        let cond = $crate::cilk_value!($builder; $( $cond )*);
...
macro_rules! cilk_ir {
    ($m:expr; define [$($ret_ty:tt)*] $name:ident [$(($($arg:tt)*)),*] { $($exp:tt)* }) => {{
        ...
        let ret_ty = $crate::cilk_parse_ty!($m.types, $($ret_ty)*);
        let args_ty = vec![$( $crate::cilk_parse_ty!($m.types, $($arg)*) ),*];
        ...
        $crate::cilk_expr!(builder; bb_map; $( $exp )*);
        f_id
    }};
    (($builder:expr) { $($exp:tt)* }) => {{
