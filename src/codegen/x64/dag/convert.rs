...
impl<'a> ConvertToDAG<'a> {
    ...
    pub fn construct_dag(&mut self, func_id: FunctionId) -> DAGFunction {
        ...
        for (i, bb_id) in func.basic_blocks.order.iter().enumerate() {
            let bb = &func.basic_blocks.arena[*bb_id];
            let id = self.construct_dag_from_basic_block(func, bb, i == 0);
    ...
    pub fn get_node_from_value(&mut self, v: &Value) -> Raw<DAGNode> {
        match v {
            ...
            Value::Argument(av) => {
                if let Some(r) = self.cur_conv_info_ref().arg_regs.get(&av.index) {
                    return *r;
                }
    ...
    fn construct_dag_from_basic_block(
        &mut self,
        func: &Function,
        bb: &BasicBlock,
        is_entry: bool,
        ...
        if is_entry {
            ...
            for i in 0..func.get_params_len() {
                if let Some(ty) = func.get_param_type(i) {
                    ...
                    let arg_reg = self.alloc_node(DAGNode::new_phys_reg(
                        &self.cur_conv_info_ref().regs_info,
                        arg_reg,
                    ));
                    let vreg = self
                        .cur_conv_info_mut()
                        .regs_info
                        .new_virt_reg(arg_reg_class);
                    ...
                    self.cur_conv_info_mut().arg_regs.insert(i, vreg);
                }
            }
        }
        ...
        for inst_val in bb.iseq_ref().iter() {
            ...
            match inst.opcode {
                ...
                Opcode::Br => {
                    let br = self.cur_conv_info_mut_with(|c| {
    ...
    fn construct_dag_for_gep(
        ...
        for idx in indices {
            let size = match ty {
                Type::Struct(id) => {
                    let off = *self.types.base.borrow().non_primitive_types[id]
                        .as_struct()
                ...
                NodeKind::Operand(OperandNodeKind::Constant(ConstantKind::Int32(i))) => {
                    heap.alloc(DAGNode::new(
                        NodeKind::Operand(OperandNodeKind::Constant(ConstantKind::Int32(
                            size.unwrap_or(i * ty.size_in_byte(&self.types) as i32),
                        ))),
                        vec![],
                        Type::Int32,
                    ))
    ...
    fn cur_conv_info_ref(&self) -> &ConversionInfo {
